...
export const getDateTime = (dateString: string | undefined): string => {
  // locale is fixed to always show day/month/year, in this order
  return dateString
    ? new Date(dateString).toLocaleString(SG_LOCALE, {
        timeZoneName: "short",
        month: "long",
        day: "numeric",
        year: "numeric",
...
export const extractInfo = (
  ...
  const swabType =
    typeof specimen?.type === "object" ? specimen?.type.coding?.[0] : undefined;
  const swabCollectionDate = getDateTime(
