  ...
  switch (resource.resourceType) {
    ...
    case "Patient":
      return {
        ...
        nationality: resource.extension?.[0].extension?.find(e => e.url === "code")?.valueCodeableConcept?.coding?.[0],
...
export const parse = (fhirBundle: R4.IBundle): Bundle => {
  ...
  const observations = fhirBundle.entry
    ...
    ?.map(o => {
      ...
      const fhirSpecimen = fhirBundle.entry?.find(entry => entry.fullUrl === observation.specimenResourceUuid)
        ?.resource;
      const specimen = parsers(fhirSpecimen) as Specimen;

      // 2b. Practitioner resource
      const fhirPractitioner = fhirBundle.entry?.find(entry => entry.fullUrl === observation.practitionerResourceUuid)
        ?.resource;
